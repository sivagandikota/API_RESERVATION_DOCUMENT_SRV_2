...
import com.sap.xi.sd_sls.SalesOrder;
...
public class SalesOrderUploadView {
	...
	private HashMap sales = new HashMap<Integer,SalesOrder>();
	private HashMap salesItems = new HashMap<Integer,SalesOrderItem>();
	private HashMap businessPartners = new HashMap<Integer,com.sap.xi.sd.Partner>();
	//private HashMap scheduleLine = new HashMap<Integer,model.ScheduleLine>();
	private HashMap addresse = new HashMap<Integer,com.sap.xi.sd.Address>();
	...
	public void init() {
        ...
        factory.setAddress("http://localhost:8080/SalesOrder/ws/SalesOrder?wsdl");
    ...
    public void process(InputStream is) {
        ...
        sales = new HashMap<Integer,SalesOrder>();
    ...
    private void addSales(Sheet sheet) {
  	  ...
  	  for(int j=sheet.getFirstRowNum()+1;j<sheet.getPhysicalNumberOfRows();j++) {
  		  ...
  		  SalesOrder salesOrd=new SalesOrder();
    ...
    private void addSalesItems(Sheet sheet) {
    	System.out.println(sheet.getSheetName()+"--"+sheet.getFirstRowNum()+"--"+sheet.getPhysicalNumberOfRows());
    	  for(int j=sheet.getFirstRowNum()+1;j<sheet.getPhysicalNumberOfRows();j++) {
    		  ...
    		  SalesOrderItem salesOrd=new SalesOrderItem();
    ...
    private void processSalesItems() {
    	...
    	if(this.salesItems_businessPartners.size()>0) {
    		...
    		while(i.hasNext()) {
    			...
    			bp=(Partner)this.businessPartners.get(key);
    			sItem=(SalesOrderItem)this.salesItems.get(salesItems_businessPartners.get(key));
    			//sItem.setPartner(bp);
    	...
    	if(this.salesItems_addresse.size()>0) {
    		...
    		while(i.hasNext()) {
    			key=Integer.valueOf(i.next().toString());
    			/*addr=(Address)this.address.get(key);
    ...
    private void processSales() {
    	...
    	if(this.sales_businessPartners.size()>0) {
    		...
    		while(i.hasNext()) {
    			key=Integer.valueOf(i.next().toString());
    			bp=(Partner)this.businessPartners.get(key);
    			sItem=(SalesOrderItem)this.salesItems.get(sales_businessPartners.get(key));
    	...
    	if(this.sales_salesItems.size()>0) {
    		...
    		while(i.hasNext()) {
    			key=Integer.valueOf(i.next().toString());
    			sItem = (SalesOrderItem)this.salesItems.get(key);
    			sale = (SalesOrder)this.sales.get(sales_salesItems.get(key));
    			sale.getItem().add(sItem);
    	...
    	if(this.sales_addresse.size()>0) {
    		...
    		while(i.hasNext()) {
    			key=Integer.valueOf(i.next().toString());
    			addr = (Address)this.salesItems.get(key);
    			sale = (SalesOrder)this.sales.get(sales_addresse.get(key));
    			//sale.getPartner();new Partner().setAddress(addr);
